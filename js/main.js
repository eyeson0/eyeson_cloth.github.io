...
/* ============================================================
   AUTH HELPERS (used by login/register pages via inline scripts)
   ============================================================ */
function eyesonLogin(email, name) {
  store.set("user", { email: email, name: name || email.split("@")[0] });
  location.href = "account.html";
}

/* ============================================================
   BROKEN LINK PROTECTION
   Any anchor pointing to a missing local page redirects home.
   ============================================================ */
function guardBrokenLinks() {
  document.addEventListener("click", (e) => {
    ...
    fetch(href, { method: "HEAD" }).then((r) => { if (r.status === 404) location.href = "index.html"; }).catch(() => {});
  });
}
